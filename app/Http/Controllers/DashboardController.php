...
final class DashboardController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $deviceOptions = $devices->map(fn (Device $device) => [
            ...
            'water_level' => $device->latestWaterLevel?->value !== null
                ? (float) $device->latestWaterLevel->value
                : null,
            'risk' => $device->latestRiskEvaluation?->risk_level ?? RiskLevel::Aman,
        ]);

        // Alat terpilih: dari ?device= bila valid, jika tidak → risiko tertinggi.
        $requested = $request->query('device');
        $selectedDevice = $devices->firstWhere('device_code', $requested)
    ...
    public function snapshot(Device $device): JsonResponse
    {
        $device->load([
            'latestWaterLevel',
            'latestTemperature',
    ...
    private function toDeviceDetail(Device $device): array
    {
        ...
        return [
            ...
            'status' => $device->status->value,
            'water_level' => $this->floatOrNull($device->latestWaterLevel?->value),
            ...
            'temperature' => $this->floatOrNull($device->latestTemperature?->value),
            'humidity' => $this->floatOrNull($device->latestHumidity?->value),
            'air_pressure' => $this->floatOrNull($device->latestAirPressure?->value),
            'wind_speed' => $this->floatOrNull($device->latestWindSpeed?->value),
            'wind_direction' => $this->floatOrNull($device->latestWindDirection?->value),
    ...
    {
        return $device->latestPredictionCurve()
            ->map(fn ($prediction) => [
                'horizon' => (int) $prediction->horizon_minutes,
                'value' => (float) $prediction->predicted_value,
                'at' => $prediction->predicted_at->timezone('Asia/Jakarta')->format('H:i'),
    ...
    private function telemetryHistory(Device $device): array
    {
        ...
        return $types
            ->mapWithKeys(fn (SensorType $type) => [
                $type->value => $rows
    ...
    private function worstRiskDevice(Collection $devices): Device
    {
        ...
        return $devices->sortByDesc(function (Device $device) use ($order) {
            $level = $device->latestRiskEvaluation?->risk_level ?? RiskLevel::Aman;
