...
<div class="flex h-full flex-col">
    <div class="flex items-center gap-3 border-b border-[var(--color-border)] px-6 py-5">
        <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--color-accent)]/15 text-[var(--color-accent)]">
        ...
        @foreach ($menu as $section)
            <div class="space-y-1">
                @if (! empty($section['group']))
                    <p class="px-3 pb-1 text-[10px] font-semibold uppercase tracking-wider text-[var(--color-text-muted)]">
                        {{ $section['group'] }}
                    </p>
                @endif
        ...
        @auth
            @php
                ...
                $initials = \Illuminate\Support\Str::of($user->name)
                    ...
                    ->take(2)
                    ->implode('');
                $itemClass = 'flex items-center gap-2.5 rounded-md px-2.5 py-2 text-sm text-[var(--color-text-muted)] transition-colors hover:bg-[var(--color-surface-2)] hover:text-[var(--color-text)]';
            @endphp
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                >
                    ...
                    <form method="POST" action="{{ Route::has('logout') ? route('logout') : '#' }}">
                        @csrf
                        <button type="submit" class="flex w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-left text-sm text-[var(--color-bahaya)] transition-colors hover:bg-[color-mix(in_srgb,var(--color-bahaya)_12%,transparent)]">
