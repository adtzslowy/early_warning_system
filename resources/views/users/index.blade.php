...
@section('breadcrumb')
    <x-breadcrumb :items="[['label' => 'Dashboard', 'href' => route('dashboard')], ['label' => 'User Managemen']]" />
@endsection
...
@section('content')
    <div class="space-y-6" x-data="{
        ...
        url: '',
        name: '',
        ask(url, name) { this.url = url;
            this.name = name;
            this.confirm = true; },
    }" @keydown.escape.window="confirm = false">

        <x-page-header title="User Managemen" description="Kelola akun pengguna dan hak akses sistem.">
            @can('create users')
                <x-slot:actions>
        ...
        <x-card padding="p-0">
            ...
            <div class="border-b border-[var(--color-border)] p-4">
                <form method="GET" action="{{ route('users') }}" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <select name="role"
                        ...
                        @foreach ($roles as $r)
                            <option value="{{ $r->name }}" @selected($role === $r->name)>{{ ucfirst($r->name) }}
                            </option>
                        @endforeach
                    </select>
            ...
            @if ($users->isEmpty())
                <x-empty-state icon="inbox" title="{{ $total === 0 ? 'Belum ada user' : 'Tidak ada hasil' }}"
                                ...
                                <tr class="transition-colors hover:bg-[var(--color-surface-2)]">
                                    <td data-label="Nama" class="px-4 py-3">
                                        <div class="flex items-center gap-3">
                                            ...
                                            @else
                                                <span
                                                    class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[var(--color-accent)] text-xs font-semibold text-[var(--color-accent-foreground)]">{{ $u->initials() }}</span>
                                            @endif
                                            <span class="inline-flex items-center gap-1 font-medium">
                                                {{ $u->name }}
                                                <x-verified-badge :user="$u" />
                                            </span>
                                        </div>
                                    </td>
                                    <td data-label="Email" class="px-4 py-3 text-[var(--color-text-muted)]">
                                        {{ $u->email }}</td>
                                    <td data-label="Role" class="px-4 py-3">
                                        @forelse ($u->roles as $r)
                                            <span
                                                class="inline-flex items-center rounded-full border border-[var(--color-border)] bg-[var(--color-surface-2)] px-2 py-0.5 text-xs font-medium">{{ ucfirst($r->name) }}</span>
                                        @empty
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="mt-1.5 text-sm text-[var(--color-text-muted)]">
                                Akun <span class="font-medium text-[var(--color-text)]" x-text="name"></span> akan dihapus
